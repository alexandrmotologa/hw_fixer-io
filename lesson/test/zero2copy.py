...
def option_4(val):
    start = datetime.datetime.strptime(year_constructor(str(input("y: ")).split()), "%Y-%m-%d")

    end = datetime.datetime.strptime(year_constructor(str(input("y: ")).split()), "%Y-%m-%d")
    ...
    try:
        ...
        while option != 0:
            ...
            if option == 2:
                disp()
            if option == 3:
                option_3(input("Introduceti valuta de care dispuneti: ").upper())
                today = datetime.datetime.now()
                ymd = (str(today)).split(' ')[0]
            if option == 4:
                option_4(input("Introduceti valuta de care dispuneti: ").upper())
    except ValueError:
        menu()
